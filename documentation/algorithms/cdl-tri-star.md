...
`CDLTriStar` is RTTA's streaming detector for the **Tri-star** candlestick pattern.
Three consecutive dojis; the middle doji’s body position sets bullish vs bearish bias.
...
Require \(\mathrm{doji}(a),\mathrm{doji}(b),\mathrm{doji}(c)\).

- If \(\mathrm{mid}_b < \mathrm{mid}_a\) and \(\mathrm{mid}_b < \mathrm{mid}_c\) → \(+100\).
- If \(\mathrm{mid}_b > \mathrm{mid}_a\) and \(\mathrm{mid}_b > \mathrm{mid}_c\) → \(-100\).
- Otherwise → \(+100\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLTriStar`,
...
- [Investopedia: Tri-star](https://www.investopedia.com/terms/t/tri-star.asp)
