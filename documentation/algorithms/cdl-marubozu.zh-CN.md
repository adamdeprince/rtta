...
`CDLMarubozu` 是 RTTA 对**光头光脚线（Marubozu）** K 线形态的流式检测器。
该 K 线的实体占据绝大部分全幅，表示很强的单向确信度。
...
若 \(\mathrm{range}>0\) 且
\(\mathrm{body}\ge0.9\cdot\mathrm{range}\)，则阳线输出 \(+100\)，阴线
输出 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLMarubozu` 中，并建立在
