...
`CDLMarubozu` is RTTA's streaming detector for the **Marubozu** candlestick pattern.
A bar whose real body dominates the range (strong directional conviction).
...
```python
value = rtta.CDLMarubozu(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLMarubozu(fillna=True).batch(open, high, low, close)
```
...
If \(\mathrm{range}>0\) and \(\mathrm{body} \ge 0.9\cdot \mathrm{range}\), emit
\(+100\) when bullish and \(-100\) when bearish.
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLMarubozu`,
