...
`CDLSpinningTop` is RTTA's streaming detector for the **Spinning top** candlestick pattern.
A small body with meaningful upper and lower shadows (indecision / churn).
...
Emit \(+100\) when \(\mathrm{range}>0\) and
\[
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLSpinningTop`,
