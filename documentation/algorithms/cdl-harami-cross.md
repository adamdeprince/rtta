...
`CDLHaramiCross` is RTTA's streaming detector for the **Harami cross** candlestick pattern.
A doji inside a relatively large prior body (stronger indecision than a plain harami).
...
```python
value = rtta.CDLHaramiCross(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLHaramiCross(fillna=True).batch(open, high, low, close)
```
...
If \(\mathrm{doji}(c)\), \(\mathrm{inside}(c,p)\), and
\(\mathrm{body}_p > 0.5\cdot \overline B\), emit \(+100\) when the prior bar is
bearish, else \(-100\) when the prior bar is bullish (neutral prior → \(+100\)).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLHaramiCross`,
...
- [Investopedia: Harami cross](https://www.investopedia.com/terms/h/haramicross.asp)
