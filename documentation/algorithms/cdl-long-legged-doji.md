...
`CDLLongLeggedDoji` is RTTA's streaming detector for the **Long-legged doji** candlestick pattern.
A doji with long upper *and* lower shadows, showing wide two-way trade around a tiny body.
...
```python
value = rtta.CDLLongLeggedDoji(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLLongLeggedDoji(fillna=True).batch(open, high, low, close)
```
...
1 bar(s) have been seen; with `fillna=True` (default), unmatched bars return `0`.
...
Emit \(+100\) when \(\mathrm{doji}(b)\), \(\mathrm{range}>0\), and
\[
\mathrm{upper} \ge 0.3\cdot \mathrm{range}
\land
\mathrm{lower} \ge 0.3\cdot \mathrm{range}.
\]
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLLongLeggedDoji`,
...
- [Investopedia: Long-legged doji](https://www.investopedia.com/terms/l/long-legged-doji.asp)
