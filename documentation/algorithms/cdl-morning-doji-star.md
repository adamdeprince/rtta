...
`CDLMorningDojiStar` is RTTA's streaming detector for the **Morning doji star** candlestick pattern.
Morning star whose middle bar is a doji (stronger indecision before the bull recovery).
...
```python
value = rtta.CDLMorningDojiStar(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLMorningDojiStar(fillna=True).batch(open, high, low, close)
```
...
Same as morning star, but the middle condition is \(\mathrm{doji}(b)\) instead of short body.
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLMorningDojiStar`,
