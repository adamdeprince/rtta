...
`CDL3WhiteSoldiers` is RTTA's streaming detector for the **Three white soldiers** candlestick pattern.
Three advancing bullish bodies with opens inside the prior body and limited upper shadows.
...
```python
value = rtta.CDL3WhiteSoldiers(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDL3WhiteSoldiers(fillna=True).batch(open, high, low, close)
```
...
Emit \(+100\) when \(a,b,c\) are all bullish,
\[
C_b>C_a,\ C_c>C_b,\quad
O_a < O_b < C_a,\quad
O_b < O_c < C_b,
\]
and each bar has \(\mathrm{smallUpper}(\cdot,0.25)\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDL3WhiteSoldiers`,
...
- [Investopedia: Three white soldiers](https://www.investopedia.com/terms/t/three-white-soldiers.asp)
