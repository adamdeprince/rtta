...
`CDLHammer` 是 RTTA 对**锤子线（Hammer）** K 线形态的流式检测器。该形态
实体较小、靠近最高价并带有长下影线，且先行趋势不是上涨，表示低价受到看涨拒绝。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLHammer` 中，并建立在共享的
