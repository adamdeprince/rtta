...
`CDLPiercing` is RTTA's streaming detector for the **Piercing line** candlestick pattern.
A bullish bar that opens below the prior close and closes through the prior midpoint but still below the prior open.
...
Emit \(+100\) when prior is bear, current is bull,
\[
O_c < C_p,\quad
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLPiercing`,
...
- [Investopedia: Piercing pattern](https://www.investopedia.com/terms/p/piercingpattern.asp)
