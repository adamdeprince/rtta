...
`CDLClosingMarubozu` is RTTA's streaming detector for the **Closing marubozu** candlestick pattern.
A strong body that closes at (or very near) the bar extreme.
...
```python
value = rtta.CDLClosingMarubozu(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLClosingMarubozu(fillna=True).batch(open, high, low, close)
```
...
If bullish, \(\mathrm{upper} \le 0.05\cdot \mathrm{range}\) and
\(\mathrm{body} \ge 0.6\cdot \mathrm{range}\) → \(+100\).

If bearish, \(\mathrm{lower} \le 0.05\cdot \mathrm{range}\) and
\(\mathrm{body} \ge 0.6\cdot \mathrm{range}\) → \(-100\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLClosingMarubozu`,
...
- [Investopedia: Marubozu](https://www.investopedia.com/terms/m/marubozu.asp)
