...
`CDLDragonflyDoji` is RTTA's streaming detector for the **Dragonfly doji** candlestick pattern.
A doji with a long lower shadow and almost no upper shadow — buyers rejected lows.
...
```python
value = rtta.CDLDragonflyDoji(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLDragonflyDoji(fillna=True).batch(open, high, low, close)
```
...
Often read as bullish rejection after a decline; RTTA does not require trend context.
...
Emit \(+100\) when
\[
\mathrm{doji}(b) \land \mathrm{longLower}(b,2) \land \mathrm{smallUpper}(b,0.1).
\]
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLDragonflyDoji`,
