...
`CDLDoji` is RTTA's streaming detector for the **Doji** candlestick pattern.
A doji has a very small real body relative to the bar range and signals balance between buyers and sellers (indecision).
...
```python
value = rtta.CDLDoji(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLDoji(fillna=True).batch(open, high, low, close)
```
...
Classic dojis appear after extended moves as pause/reversal *candidates*; RTTA only tests geometry.
...
Let \(b\) be the newest bar. Emit \(+100\) when \(\mathrm{doji}(b)\); else \(0\).
(Doji is non-directional in this implementation.)
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLDoji`,
