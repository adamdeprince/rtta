...
`CDLMorningStar` is RTTA's streaming detector for the **Morning star** candlestick pattern.
Three-bar bullish reversal: long bear, small middle, long bull closing above the first midpoint.
...
Let \(a=\mathrm{at}(2)\), \(b=\mathrm{at}(1)\), \(c=\mathrm{at}(0)\), \(\overline B\) average body.

Emit \(+100\) when
\[
a\ \mathrm{bear}\land \mathrm{longBody}(a,\overline B,0.8)
\land \mathrm{shortBody}(b,\overline B,0.6)
\land c\ \mathrm{bull}\land \mathrm{longBody}(c,\overline B,0.7)
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLMorningStar`,
