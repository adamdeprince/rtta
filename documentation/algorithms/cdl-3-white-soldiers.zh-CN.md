...
`CDL3WhiteSoldiers` 是 RTTA 对**三白兵（红三兵）** K 线形态的流式检测器。
该形态由三根连续走高的阳线实体组成，每根开盘价位于前一根实体内部，且上影线较短。
...
且每根 K 线均满足 \(\mathrm{smallUpper}(\cdot,0.25)\) 时，输出 \(+100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDL3WhiteSoldiers`
