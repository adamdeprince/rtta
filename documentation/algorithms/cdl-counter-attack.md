...
`CDLCounterAttack` is RTTA's streaming detector for the **Counterattack** candlestick pattern.
Opposite-color long bodies that finish near the same close (sharp rebuttal bar).
...
Both bars need \(\mathrm{longBody}(\cdot,\overline B,0.8)\) and
\(|C_c-C_p| \le 0.1\cdot \overline R\).

- Prior bear + current bull → \(+100\).
- Prior bull + current bear → \(-100\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLCounterAttack`,
