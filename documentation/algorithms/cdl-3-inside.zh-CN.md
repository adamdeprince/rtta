...
`CDL3Inside` 是 RTTA 对**三内升/三内降** K 线形态的流式检测器。
该形态先出现孕线组合，随后一根确认 K 线的收盘价越过第一根 K 线的开盘价。
...
- \(+100\)：\(a\) 为阴线，\(b\) 为阳线，满足
  \(\mathrm{inside}(b,a)\)，\(c\) 为阳线，且 \(C_c>O_a\)。
- \(-100\)：\(a\) 为阳线，\(b\) 为阴线且位于内部，\(c\) 为阴线，且
  \(C_c<O_a\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDL3Inside` 中，并建立在
