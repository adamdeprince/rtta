...
`CDLShortLine` 是 RTTA 对**短实体线（Short line）**K 线形态的流式检测器。
其实体相对于近期平均实体较小。
...
```python
value = rtta.CDLShortLine(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLShortLine(fillna=True).batch(open, high, low, close)
```
...
当
\(\mathrm{shortBody}(b,\overline B,0.5)\) 且 \(\mathrm{range}>0\) 时，
按 K 线方向输出 \(+100\) 或 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLShortLine` 中，并建立在共享的
`CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的几何规则，
**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：K 线基础](https://www.investopedia.com/terms/c/candlestick.asp)
