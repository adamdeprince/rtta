...
`CDLLongLeggedDoji` 是 RTTA 对**长脚十字线（Long-Legged Doji）** K 线
形态的流式检测器。该十字线同时带有较长的上影线和下影线，表示价格围绕很小的
实体发生了大范围双向交易。
...
当 \(\mathrm{doji}(b)\)、\(\mathrm{range}>0\) 均成立，且满足
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLLongLeggedDoji`
