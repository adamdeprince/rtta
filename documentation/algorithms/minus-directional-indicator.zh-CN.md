...
`MinusDirectionalIndicator` 计算负方向指标 \(DI^-\)。
...
该指标属于 Wilder 方向运动体系。更新时比较当前 K 线相对上一根 K 线的高低点延伸，平滑负方向运动和真实波幅。
...
递推公式在 `src/rtta/indicator.cpp` 的 `class MinusDirectionalIndicator` 中实现。
