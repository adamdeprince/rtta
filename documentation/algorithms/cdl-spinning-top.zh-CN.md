...
`CDLSpinningTop` 是 RTTA 对**纺锤线（Spinning top）**K 线形态的流式
检测器。其实体较小，同时具有明显的上影线和下影线，表示犹豫或反复换手。
...
```python
value = rtta.CDLSpinningTop(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLSpinningTop(fillna=True).batch(open, high, low, close)
```
...
将 \((O_t,H_t,L_t,C_t)\) 压入环形缓冲区。只有其中至少已有 **1** 根 K 线
时才进行检测。

当 \(\mathrm{range}>0\) 且

\[
\mathrm{body} \le 0.3\cdot \mathrm{range}
\land
\mathrm{upper},\mathrm{lower} \ge 0.2\cdot \mathrm{range}.
\]
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLSpinningTop` 中，并建立在
共享的 `CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的
几何规则，**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：纺锤线](https://www.investopedia.com/terms/s/spinning-top.asp)
