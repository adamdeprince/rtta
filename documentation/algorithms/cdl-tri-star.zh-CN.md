...
`CDLTriStar` 是 RTTA 对**三星十字（Tri-star）**K 线形态的流式检测器。
该形态由连续三根十字线构成，中间十字线的实体位置决定看涨或看跌倾向。
...
```python
value = rtta.CDLTriStar(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLTriStar(fillna=True).batch(open, high, low, close)
```
...
首先要求 \(\mathrm{doji}(a)\)、\(\mathrm{doji}(b)\) 和
\(\mathrm{doji}(c)\) 同时成立。

- 若 \(\mathrm{mid}_b < \mathrm{mid}_a\) 且 \(\mathrm{mid}_b < \mathrm{mid}_c\)，则输出 \(+100\)。
- 若 \(\mathrm{mid}_b > \mathrm{mid}_a\) 且 \(\mathrm{mid}_b > \mathrm{mid}_c\)，则输出 \(-100\)。
- 否则输出 \(+100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLTriStar` 中，并建立在共享的
`CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的几何规则，
**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：三星十字](https://www.investopedia.com/terms/t/tri-star.asp)
