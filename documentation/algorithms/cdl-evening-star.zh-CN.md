...
`CDLEveningStar` 是 RTTA 对**黄昏星（Evening Star）** K 线形态的流式
检测器。这是一种三 K 线看跌反转：先是长阳线，再是小实体，最后是一根收盘低于
第一根实体中点的长阴线。
...
当 \(a\) 为长阳线、\(b\) 为短实体、\(c\) 为长阴线，且
\(C_c<\mathrm{mid}_a\) 时，输出 \(-100\)。长短实体使用与启明星相同的
\(\overline B\) 倍数。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLEveningStar` 中，并建立在
