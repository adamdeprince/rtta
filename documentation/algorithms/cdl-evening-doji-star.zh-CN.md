...
`CDLEveningDojiStar` 是 RTTA 对**十字黄昏星（Evening Doji Star）** K 线
形态的流式检测器，即中间一根 K 线为十字线的黄昏星。
...
其规则与黄昏星相同，但中间 K 线必须满足 \(\mathrm{doji}(b)\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLEveningDojiStar`
