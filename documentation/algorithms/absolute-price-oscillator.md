...
`AbsolutePriceOscillator` is RTTA's streaming implementation of: Difference between fast and slow moving averages in price units.
...
`AbsolutePriceOscillator` is a causal smoother or average. It updates compact rolling or exponential state with the newest observation and returns the current smoothed estimate.
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class AbsolutePriceOscillator`.
