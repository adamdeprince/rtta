...
`CDLHaramiCross` 是 RTTA 对**十字孕线（Harami Cross）** K 线形态的流式
检测器。当前十字线位于前一根相对较大的实体内部，比普通孕线体现出更强的犹豫。
...
若 \(\mathrm{doji}(c)\)、\(\mathrm{inside}(c,p)\) 均成立，且
\(\mathrm{body}_p>0.5\cdot\overline B\)，则前一根为阴线时输出 \(+100\)，
前一根为阳线时输出 \(-100\)；前一根方向中性时输出 \(+100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLHaramiCross` 中，并建立在
