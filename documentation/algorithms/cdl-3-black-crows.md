...
`CDL3BlackCrows` is RTTA's streaming detector for the **Three black crows** candlestick pattern.
Three declining bearish bodies with opens inside the prior body and limited lower shadows.
...
```python
value = rtta.CDL3BlackCrows(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDL3BlackCrows(fillna=True).batch(open, high, low, close)
```
...
Emit \(-100\) when \(a,b,c\) are all bearish with falling closes, opens inside prior bodies,
and each bar has \(\mathrm{smallLower}(\cdot,0.25)\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDL3BlackCrows`,
...
- [Investopedia: Three black crows](https://www.investopedia.com/terms/t/three-black-crows.asp)
