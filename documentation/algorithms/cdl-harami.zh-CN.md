...
`CDLHarami` 是 RTTA 对**孕线（Harami）** K 线形态的流式检测器。一根较小、
颜色相反的实体位于前一根较大实体“内部”，表示行情暂停或可能反转。
...
```python
value = rtta.CDLHarami(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLHarami(fillna=True).batch(open, high, low, close)
```
...
- 前一根为阴线、当前为阳线，满足 \(\mathrm{inside}(c,p)\)，且
  \(\mathrm{body}_p>\mathrm{body}_c\) 时，输出 \(+100\)。
- 前一根为阳线、当前为阴线并位于其内部，且前一根实体较大时，输出 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLHarami` 中，并建立在共享的
`CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的几何规则，
**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：Harami](https://www.investopedia.com/terms/h/harami.asp)
