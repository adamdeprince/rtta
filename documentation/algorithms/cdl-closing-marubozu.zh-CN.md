...
`CDLClosingMarubozu` 是 RTTA 对**收盘光头光脚线（Closing Marubozu）**
K 线形态的流式检测器。该形态具有强劲实体，并收盘于整根 K 线的极端位置或其
非常近的位置。
...
调用 `update` 一致。当 `fillna=False` 时，在至少取得 1 根 K 线前返回 NaN；
...
将 \((O_t,H_t,L_t,C_t)\) 压入环形缓冲区。至少已有 **1** 根 K 线时即可
进行检测。

若为阳线，且 \(\mathrm{upper}\le0.05\cdot\mathrm{range}\)、
\(\mathrm{body}\ge0.6\cdot\mathrm{range}\)，则输出 \(+100\)。

若为阴线，且 \(\mathrm{lower}\le0.05\cdot\mathrm{range}\)、
\(\mathrm{body}\ge0.6\cdot\mathrm{range}\)，则输出 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLClosingMarubozu`
中，并建立在共享的 `CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合
流式计算的几何规则，**并非**对 TA-Lib 每一张平均实体回看表逐行移植。
