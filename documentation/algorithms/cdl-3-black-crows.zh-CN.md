...
`CDL3BlackCrows` 是 RTTA 对**三只乌鸦** K 线形态的流式检测器。
该形态由三根连续走低的阴线实体组成，每根开盘价位于前一根实体内部，且下影线较短。
...
当 \(a,b,c\) 均为阴线、收盘价依次下降、每根开盘价位于前一根实体内部，且
每根 K 线均满足 \(\mathrm{smallLower}(\cdot,0.25)\) 时，输出 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDL3BlackCrows` 中，
并建立在共享的 `CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式
计算的几何规则，**并非**对 TA-Lib 每一张平均实体回看表逐行移植。
