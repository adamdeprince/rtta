...
`CDLHangingMan` is RTTA's streaming detector for the **Hanging man** candlestick pattern.
Hammer geometry after a non-down prior trend (bearish warning at highs).
...
```python
value = rtta.CDLHangingMan(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLHangingMan(fillna=True).batch(open, high, low, close)
```
...
Same body/shadow rules as the hammer; only the trend filter differs.
...
Emit \(-100\) when the hammer geometry holds and \(\tau \ge 0\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLHangingMan`,
...
- [Investopedia: Hanging man](https://www.investopedia.com/terms/h/hangingman.asp)
