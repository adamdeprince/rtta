...
`CDLPiercing` 是 RTTA 对**刺透线（Piercing line）**K 线形态的流式检测器。
当前阳线低开于前一根 K 线的收盘价，收盘越过前一根实体的中点，但仍低于前一根
K 线的开盘价。
...
```python
value = rtta.CDLPiercing(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLPiercing(fillna=True).batch(open, high, low, close)
```
...
当 \(C_t \ge O_t\) 时为**阳线**，当 \(C_t < O_t\) 时为**阴线**。
K 线保存在一个短环形缓冲区中；年龄 \(0\) 表示最新 K 线，年龄 \(1\) 表示
前一根，以此类推。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLPiercing` 中，并建立在共享的
`CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的几何规则，
**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：刺透形态](https://www.investopedia.com/terms/p/piercingpattern.asp)
