...
`CDLInvertedHammer` is RTTA's streaming detector for the **Inverted hammer** candlestick pattern.
Small body near the low with a long upper shadow in a non-up prior trend (bullish probe of highs after a decline).
...
```python
value = rtta.CDLInvertedHammer(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLInvertedHammer(fillna=True).batch(open, high, low, close)
```
...
Emit \(+100\) when
\[
\mathrm{longUpper}(b,2)
\land \mathrm{smallLower}(b,0.15)
\land 0 < \mathrm{body} \le 0.4\cdot \mathrm{range}
\land \tau \le 0.
\]
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLInvertedHammer`,
...
- [Investopedia: Inverted hammer](https://www.investopedia.com/terms/i/invertedhammer.asp)
