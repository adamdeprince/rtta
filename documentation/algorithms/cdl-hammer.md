...
`CDLHammer` is RTTA's streaming detector for the **Hammer** candlestick pattern.
A small body near the high with a long lower shadow, in a non-up prior trend (bullish rejection of lows).
...
```python
value = rtta.CDLHammer(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLHammer(fillna=True).batch(open, high, low, close)
```
...
Shares geometry with the hanging man; the prior-trend filter separates the labels.
...
Emit \(+100\) when
\[
\mathrm{longLower}(b,2)
\land \mathrm{smallUpper}(b,0.15)
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLHammer`,
