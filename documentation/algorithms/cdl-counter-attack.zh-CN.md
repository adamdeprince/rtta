...
`CDLCounterAttack` 是 RTTA 对**反击线（Counterattack）** K 线形态的流式
检测器。该形态由颜色相反的两根长实体构成，且两者收盘价非常接近，表现为对前一根
K 线的强烈反驳。
...
```python
value = rtta.CDLCounterAttack(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLCounterAttack(fillna=True).batch(open, high, low, close)
```
...
两根 K 线都必须满足 \(\mathrm{longBody}(\cdot,\overline B,0.8)\)，并且
\(|C_c-C_p|\le0.1\cdot\overline R\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLCounterAttack`
