...
`CDLDragonflyDoji` 是 RTTA 对**蜻蜓十字线（Dragonfly Doji）** K 线形态的
流式检测器。该形态为带长下影线、几乎没有上影线的十字线，表示买方拒绝了低价。
...
它常被解读为下跌后的看涨拒绝信号；RTTA 不要求存在先行趋势背景。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLDragonflyDoji`
