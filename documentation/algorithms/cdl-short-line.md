...
`CDLShortLine` is RTTA's streaming detector for the **Short line** candlestick pattern.
A small real body versus the recent average body.
...
Emit \(+100\) / \(-100\) by direction when
\(\mathrm{shortBody}(b,\overline B,0.5)\) and \(\mathrm{range}>0\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLShortLine`,
