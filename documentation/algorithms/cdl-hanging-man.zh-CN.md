...
`CDLHangingMan` 是 RTTA 对**上吊线（Hanging Man）** K 线形态的流式检测器。
它是在先行趋势不为下跌时出现的锤子线几何结构，是高位的看跌警告。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLHangingMan` 中，并建立在
