...
`CDLDarkCloudCover` 是 RTTA 对**乌云盖顶（Dark Cloud Cover）** K 线形态
的流式检测器。当前阴线开盘高于前收盘价，收盘向下穿过前一根实体中点，但仍高于
前一根开盘价。
...
```python
value = rtta.CDLDarkCloudCover(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLDarkCloudCover(fillna=True).batch(open, high, low, close)
```
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLDarkCloudCover`
