...
`CDLEngulfing` 是 RTTA 对**吞没（Engulfing）** K 线形态的流式检测器。
当前实体以相反颜色完整吞没前一根实体，形成经典的双 K 线反转压力结构。
...
调用 `update` 一致。当 `fillna=False` 时，在至少取得 2 根 K 线前返回 NaN；
...
看涨吞没由前一根阴线和当前吞没它的阳线构成；看跌吞没则完全相反。
...
将 \((O_t,H_t,L_t,C_t)\) 压入环形缓冲区。只有其中至少已有 **2** 根 K 线
时才进行检测。

令 \(c=\mathrm{at}(0)\)、\(p=\mathrm{at}(1)\)。

- 若 \(c\) 为阳线、\(p\) 为阴线，且 \(\mathrm{engulf}(c,p)\)，则输出
  \(+100\)。
- 若 \(c\) 为阴线、\(p\) 为阳线，且 \(\mathrm{engulf}(c,p)\)，则输出
  \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLEngulfing` 中，并建立在
