...
`CDLGravestoneDoji` is RTTA's streaming detector for the **Gravestone doji** candlestick pattern.
A doji with a long upper shadow and almost no lower shadow — sellers rejected highs.
...
```python
value = rtta.CDLGravestoneDoji(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLGravestoneDoji(fillna=True).batch(open, high, low, close)
```
...
Often read as bearish rejection after a rally; RTTA emits \(-100\) on geometry alone.
...
Emit \(-100\) when
\[
\mathrm{doji}(b) \land \mathrm{longUpper}(b,2) \land \mathrm{smallLower}(b,0.1).
\]
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLGravestoneDoji`,
