...
`CDLLongLine` 是 RTTA 对**长实体线（Long Line）** K 线形态的流式检测器。
其实体相对于近期平均实体较大，同时仍占整根 K 线全幅的较大比例。
...
令 \(\overline B\) 为最近最多五根 K 线的平均实体长度。当满足
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLLongLine` 中，并建立在
