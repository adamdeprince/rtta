...
`CDLMorningDojiStar` 是 RTTA 对**十字启明星（Morning Doji Star）** K 线
形态的流式检测器，即中间一根 K 线为十字线的启明星，在多头恢复之前表现出更强的
犹豫。
...
其规则与启明星相同，但中间 K 线必须满足 \(\mathrm{doji}(b)\)，而不是短实体
条件。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLMorningDojiStar`
