...
`CDLHarami` is RTTA's streaming detector for the **Harami** candlestick pattern.
A smaller body “inside” a larger prior body of opposite color (pause / possible reversal).
...
- \(+100\) if prior bear, current bull, \(\mathrm{inside}(c,p)\), and
  \(\mathrm{body}_p > \mathrm{body}_c\).
- \(-100\) if prior bull, current bear, inside, and prior body larger.
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLHarami`,
