...
`CDLEveningStar` is RTTA's streaming detector for the **Evening star** candlestick pattern.
Three-bar bearish reversal: long bull, small middle, long bear closing below the first midpoint.
...
```python
value = rtta.CDLEveningStar(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLEveningStar(fillna=True).batch(open, high, low, close)
```
...
Emit \(-100\) when \(a\) is long bull, \(b\) short, \(c\) long bear, and \(C_c < \mathrm{mid}_a\)
(with the same \(\overline B\) factors as the morning star).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLEveningStar`,
