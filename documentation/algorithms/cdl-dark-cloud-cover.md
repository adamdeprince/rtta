...
`CDLDarkCloudCover` is RTTA's streaming detector for the **Dark cloud cover** candlestick pattern.
A bearish bar that opens above the prior close and closes through the prior midpoint but still above the prior open.
...
Emit \(-100\) when prior is bull, current is bear,
\[
O_c > C_p,\quad
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLDarkCloudCover`,
