...
`CDL3Inside` is RTTA's streaming detector for the **Three inside up/down** candlestick pattern.
A harami followed by a confirmation close beyond the first bar’s open.
...
```python
value = rtta.CDL3Inside(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDL3Inside(fillna=True).batch(open, high, low, close)
```
...
- \(+100\): \(a\) bear, \(b\) bull, \(\mathrm{inside}(b,a)\), \(c\) bull, \(C_c > O_a\).
- \(-100\): \(a\) bull, \(b\) bear, inside, \(c\) bear, \(C_c < O_a\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDL3Inside`,
