...
`CDLDojiStar` 是 RTTA 对**十字星（Doji Star）** K 线形态的流式检测器。
该形态在一根长实体后出现十字线，常以星线形式出现在单向推进之后，提示反转风险。
...
必须同时满足 \(\mathrm{doji}(c)\) 与
\(\mathrm{longBody}(p,\overline B,0.8)\)。

- 若前一根为阳线：优先匹配向上跳空的星线
  \(\mathrm{bot}_c>\mathrm{top}_p\)，输出 \(-100\)；即使没有跳空也输出
  \(-100\)。
- 若前一根为阴线：优先匹配向下跳空的星线
  \(\mathrm{top}_c<\mathrm{bot}_p\)，输出 \(+100\)；即使没有跳空也输出
  \(+100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLDojiStar` 中，并建立在
共享的 `CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的
几何规则，**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：Doji Star](https://www.investopedia.com/terms/d/doji-star.asp)
