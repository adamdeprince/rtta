...
`CDLHighWave` is RTTA's streaming detector for the **High-wave candle** candlestick pattern.
A tiny body with at least one very long shadow (volatile indecision).
...
```python
value = rtta.CDLHighWave(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLHighWave(fillna=True).batch(open, high, low, close)
```
...
Emit \(+100\) when
\[
\mathrm{body} \le 0.2\cdot \mathrm{range}
\land
\bigl(\mathrm{upper} \ge 0.4\cdot \mathrm{range}
  \lor \mathrm{lower} \ge 0.4\cdot \mathrm{range}\bigr).
\]
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLHighWave`,
