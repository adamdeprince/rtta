...
`CDLGravestoneDoji` 是 RTTA 对**墓碑十字线（Gravestone Doji）** K 线形态
的流式检测器。该形态为带长上影线、几乎没有下影线的十字线，表示卖方拒绝了高价。
...
它常被解读为上涨后的看跌拒绝信号；RTTA 仅根据几何结构输出 \(-100\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLGravestoneDoji`
