...
`CDLBeltHold` is RTTA's streaming detector for the **Belt hold** candlestick pattern.
Opens near the extreme and posts a long body (yokate / bullish or bearish belt).
...
```python
value = rtta.CDLBeltHold(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLBeltHold(fillna=True).batch(open, high, low, close)
```
...
Bullish (\( +100 \)): bullish bar, \(\mathrm{longBody}(b,\overline B,1)\), and
\(\mathrm{lower} \le 0.1\cdot \mathrm{range}\).

Bearish (\( -100 \)): bearish bar, long body, and
\(\mathrm{upper} \le 0.1\cdot \mathrm{range}\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLBeltHold`,
...
- [Investopedia: Belt hold](https://www.investopedia.com/terms/b/belthold.asp)
