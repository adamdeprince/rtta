...
`CDLShootingStar` 是 RTTA 对**流星线（Shooting star）**K 线形态的流式
检测器。它是在先行趋势并非下行时出现的倒锤头几何结构，表示高价受到看跌拒绝。
...
```python
value = rtta.CDLShootingStar(fillna=True).update(open, high, low, close)
# 看涨匹配为 +100；有方向的看跌匹配为 -100；未匹配为 0
batch = rtta.CDLShootingStar(fillna=True).batch(open, high, low, close)
```
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLShootingStar` 中，并建立在
共享的 `CdlHistory` 和 `cdl::` 几何辅助函数之上。阈值采用适合流式计算的
几何规则，**并非**对 TA-Lib 每一张平均实体回看表逐行移植。

## 参考资料

- [Investopedia：流星线](https://www.investopedia.com/terms/s/shootingstar.asp)
