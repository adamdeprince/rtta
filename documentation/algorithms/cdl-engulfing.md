...
`CDLEngulfing` is RTTA's streaming detector for the **Engulfing** candlestick pattern.
The current real body fully engulfs the prior real body with opposite color (classic two-bar reversal pressure).
...
```python
value = rtta.CDLEngulfing(fillna=True).update(open, high, low, close)
# +100 bullish match, -100 bearish match (when directional), 0 no match
batch = rtta.CDLEngulfing(fillna=True).batch(open, high, low, close)
```
...
Bullish engulfing: prior bear, current bull, engulf. Bearish: the reverse.
...
Let \(c=\mathrm{at}(0)\), \(p=\mathrm{at}(1)\).

- \(+100\) if \(c\) bull, \(p\) bear, and \(\mathrm{engulf}(c,p)\).
- \(-100\) if \(c\) bear, \(p\) bull, and \(\mathrm{engulf}(c,p)\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLEngulfing`,
...
- [Investopedia: Bullish engulfing](https://www.investopedia.com/terms/b/bullishengulfingpattern.asp)
- [Investopedia: Bearish engulfing](https://www.investopedia.com/terms/b/bearishengulfing.asp)
