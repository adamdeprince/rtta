...
`CDL3Outside` 是 RTTA 对**三外升/三外降** K 线形态的流式检测器。
该形态先出现吞没组合，随后一根确认 K 线的收盘价越过吞没 K 线的收盘价。
...
- \(+100\)：\(a\) 为阴线，\(b\) 为阳线，满足
  \(\mathrm{engulf}(b,a)\)，\(c\) 为阳线，且 \(C_c>C_b\)。
- \(-100\)：\(a\) 为阳线，\(b\) 为阴线并吞没 \(a\)，\(c\) 为阴线，且
  \(C_c<C_b\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDL3Outside` 中，并建立在
