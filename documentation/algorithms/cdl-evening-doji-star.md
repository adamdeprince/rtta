...
`CDLEveningDojiStar` is RTTA's streaming detector for the **Evening doji star** candlestick pattern.
Evening star whose middle bar is a doji.
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLEveningDojiStar`,
