...
`CDLBeltHold` 是 RTTA 对**捉腰带线（Belt Hold）** K 线形态的流式检测器。
该形态开盘于价格区间的极端附近，随后形成长实体，可为看涨或看跌捉腰带线。
...
看涨（\(+100\)）：当前为阳线，满足
\(\mathrm{longBody}(b,\overline B,1)\)，且
\(\mathrm{lower}\le0.1\cdot\mathrm{range}\)。

看跌（\(-100\)）：当前为阴线且实体较长，并满足
\(\mathrm{upper}\le0.1\cdot\mathrm{range}\)。
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLBeltHold` 中，并建立在
