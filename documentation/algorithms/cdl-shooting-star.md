...
`CDLShootingStar` is RTTA's streaming detector for the **Shooting star** candlestick pattern.
Inverted-hammer geometry after a non-down prior trend (bearish rejection of highs).
...
Emit \(-100\) when inverted-hammer geometry holds and \(\tau \ge 0\).
...
The recurrence is implemented in `src/rtta/indicator.cpp` in `class CDLShootingStar`,
