...
`CDLDoji` 是 RTTA 对**十字线（Doji）** K 线形态的流式检测器。十字线的实体
相对于整根 K 线全幅非常小，表示买卖双方力量平衡，即市场处于犹豫状态。
...
经典十字线出现在一段延伸行情之后时，可视为暂停或反转的*候选*信号；RTTA 只
检验几何结构。
...
令 \(b\) 为最新 K 线。若 \(\mathrm{doji}(b)\) 成立，则输出 \(+100\)，
否则输出 \(0\)。（本实现中的十字线不带方向。）
...
递推过程实现在 `src/rtta/indicator.cpp` 的 `class CDLDoji` 中，并建立在共享的
